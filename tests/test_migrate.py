...
from migrate import migrate_7_to_8, if_exists_move_to, migrate_8_to_9
...
class Test(TestCase):
    ...
    def test_migrate_sample_config_7_8(self):
        in_file = '../config-files-json/ors-config-sample.json'

        migrate_7_to_8(join(dirname(__file__), in_file), "./outputs/ors-config-sample-7-8-migrated.yml")

    def test_migrate_test_config_7_8(self):
        in_file = '../config-files-json/test-config.json'

        migrate_7_to_8(join(dirname(__file__), in_file), "./outputs/test-config-7-8-migrated.yml")

    def test_migrate_production_config_7_8(self):
        in_file = '../config-files-json/ors-config.driving-car.json'

        migrate_7_to_8(join(dirname(__file__), in_file), "./outputs/ors-config.driving-car-7-8-migrated.yml")
    ...
    def test_migrate_sample_config_8_9(self):
        in_file = '../config-files-yaml/ors-config-source-8.yml'

        migrate_8_to_9(join(dirname(__file__), in_file), "./outputs/ors-config-sample-8-9-migrated.yml")
